...
//set Pnuematics
        ...
        claw = new Solenoid(PneumaticsModuleType.CTREPCM, 2);
...
//arm brake logic
        if (ARM_ENCODER.getPosition() <=setPointA+0.8 && ARM_ENCODER.getPosition() >=setPointA-0.8 && WRIST_ENCODER.getPosition() <= setPointW+0.8 && WRIST_ENCODER.getPosition() >= setPointW-0.8) {
                ARM_Brake.set(false);
        }  else {
                ARM_Brake.set(true);
        }
